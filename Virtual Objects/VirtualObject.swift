...
//  RubenARIntro
//
//  Created by Ruben Glapa on 4/9/18.
...
class VirtualObject: SCNReferenceNode {
    ...
    /**
     Set the object's position based on the privided position relative to the `cameraTransform`.
     If `smoothMovement` is true, the new position will be averaged with previous position to
     avoid large jumps
 
     - Tag: VirtObjectSetPosition
    */
    func setTransform(_ newTransform: float4x4,
        ...
        /*
         ...
         updates. Notice taht the distance is applied to the vector from
         the camera to the content, so it affects only the perceived distance to the
